...
impl CfrSolver {
    /// Create a solver for the given tree. Allocates regret/strategy storage
    ...
    pub fn new(tree: GameTree) -> Self {
        ...
        let terminal_evs = terminal_ev_table();
    ...
    pub fn run_iteration(&mut self) {
        ...
        self.cfr_traverse(0, 1.0, 1.0, t);
    }

    /// Recursive CFR+ traversal. Returns EV from IP's perspective.
    ///
    /// `reach_ip`  — probability that IP reaches this node under their strategy
    /// `reach_oop` — probability that OOP reaches this node under their strategy
    fn cfr_traverse(&mut self, node_id: NodeId, reach_ip: f64, reach_oop: f64, t: u64) -> f64 {
        // Read node data without holding a borrow on self.tree
        let node_info = self.read_node(node_id);

        match node_info {
            NodeInfo::Terminal => self.terminal_evs[&node_id],
            ...
            NodeInfo::Decision { infoset_id, player, children } => {
                ...
                for (i, &child_id) in children.iter().enumerate() {
                    ...
                    let ev = self.cfr_traverse(child_id, new_reach_ip, new_reach_oop, t);
                    child_evs.push(ev);
                }
                ...
                self.storage.update_regrets(infoset_id, &cf_values);
                self.storage.accumulate_strategy(infoset_id, &strategy, t);
    ...
    /// Extract the minimal node information needed for traversal, avoiding
    /// long-lived borrows on `self.tree` during recursive calls.
    fn read_node(&self, node_id: NodeId) -> NodeInfo {
        match self.tree.get(node_id).expect("invalid node id") {
